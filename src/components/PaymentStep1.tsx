...
import { CreditCard, Wallet, ArrowRight, ClipboardList, Info, Sparkles, ReceiptText, ShieldAlert } from 'lucide-react';
...
export function PaymentStep1({ invoice, setInvoice, onSelectMethod }: PaymentStep1Props) {
  ...
  return (
    <div className="w-full bg-slate-50 px-5 py-4 pb-10 relative">
      ...
      >
        ...
        <h2 className="text-xl font-extrabold text-slate-800 mt-2 tracking-tight">
          Pilih Metode Pembayaran
        </h2>
        <p className="text-xs text-slate-500 mt-1.5 px-3 leading-relaxed">
      ...
      >
        ...
        {isEditingInvoice ? (
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <div className="mt-2 bg-gradient-to-r from-blue-900 via-blue-950 to-slate-900 rounded-xl p-3 text-white flex items-center justify-between shadow-xs">
              <div>
      ...
      <motion.div
        ...
        whileTap={{ scale: 0.98 }}
        onClick={() => onSelectMethod(PaymentMethodType.BRI)}
        className="w-full group bg-white hover:bg-slate-50 border-2 border-slate-100 rounded-3xl mb-4 p-5 flex items-center justify-between cursor-pointer transition-all duration-200"
        id="payment-card-bri"
      >
        <div className="flex items-center gap-4">
          ...
          <div className="w-12 h-12 rounded-2xl bg-blue-50/80 border border-blue-100 flex items-center justify-center p-2.5 shrink-0 transition-colors">
            ...
            <svg viewBox="0 0 100 30" className="w-10 h-10 animate-pulse" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <button
          className="px-4 py-2 bg-slate-900 group-hover:bg-blue-600 text-white text-[11px] font-bold rounded-xl flex items-center gap-1 transition-all duration-200 uppercase tracking-wider"
          id="btn-select-bri"
        >
          Pilih
      ...
      <motion.div
        ...
        whileTap={{ scale: 0.98 }}
        onClick={() => onSelectMethod(PaymentMethodType.DANA)}
        className="w-full group bg-white hover:bg-slate-50 border-2 border-slate-100 rounded-3xl mb-4 p-5 flex items-center justify-between cursor-pointer transition-all duration-200"
      ...
      >
        ...
        {/* Action picker Button */}
        <button
          className="px-4 py-2 bg-slate-900 group-hover:bg-slate-800 text-white text-[11px] font-bold rounded-xl flex items-center gap-1 transition-all duration-200 uppercase tracking-wider"
          id="btn-select-dana"
        >
          Pilih
          <ArrowRight className="w-3 h-3" />
